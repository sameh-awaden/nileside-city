...
func _ready() -> void:
    ...
    _sprite.scale = Vector2.ONE * visual_scale * 1.65
    _sprite.position.y = -maxf(15.0, (_sprite.texture.get_height() if _sprite.texture else 100) * visual_scale * 1.65 * 0.35)
    match resource_type:
        "stone":
...
func _respawn() -> void:
    ...
    if _sprite:
        ...
        var tween := create_tween()
        tween.tween_property(_sprite, "scale", Vector2.ONE * visual_scale * 1.65, 0.35).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
    queue_redraw()
